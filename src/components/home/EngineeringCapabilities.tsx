...
export const EngineeringCapabilities: React.FC = () => {
  ...
  return (
    <section className="py-16 sm:py-24 bg-slate-100/70 text-slate-900 relative overflow-hidden border-b border-slate-200">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {capabilities.map((item, index) => (
            <div
              key={index}
              className="p-6 sm:p-8 rounded-2xl bg-white border border-slate-200 hover:border-amber-300 transition-all hover:shadow-lg flex flex-col sm:flex-row gap-5"
            >
              <div className="p-3.5 bg-amber-100 rounded-2xl border border-amber-200 h-fit w-fit flex-shrink-0">
                {item.icon}
              </div>
        ...
        <div className="mt-12 p-6 rounded-2xl bg-white border border-amber-200 shadow-md flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <Award className="w-8 h-8 text-amber-600 flex-shrink-0" />
            <div>
              <h4 className="text-base font-bold text-slate-900">Quality Assurance & Compliance</h4>
              <p className="text-xs text-slate-600">Strict dimension checking, weld inspection, and galvanizing thickness verification on every production lot.</p>
            </div>
          </div>
          <div className="flex flex-wrap gap-2 text-xs font-bold text-amber-900">
            {COMPANY_CONFIG.manufacturing.standards.map((std, idx) => (
              <span key={idx} className="px-3 py-1.5 bg-amber-50 border border-amber-200 rounded-lg">
                {std}
              </span>
            ))}
          </div>
